...
# --------------------------------------------------------------------------
# synthetic topic post
#
# The published fixture predates Graze topics. This checked-in overlay gives
# local seeds one stable post that exercises topic parsing through the real
# megastream ingestion binary.
# --------------------------------------------------------------------------


def test_topic_post_fixture_contains_the_ranking_score():
    fixture = rebase.load_topic_post_fixture()
    topics = fixture["inferences"]["text"]["message.commit.record.text"]["topic"]

    assert topics["News & Social Concern"] == 0.73779296875


def test_write_topic_post_fixture_creates_a_production_shaped_archive(tmp_path, monkeypatch):
    monkeypatch.setattr(rebase, "OUT_DIR", tmp_path / "out")
    rebase.OUT_DIR.mkdir()
    fixture = rebase.load_topic_post_fixture()
    created_at = dt.datetime(2026, 7, 22, 10, 0, 1, tzinfo=dt.UTC)

    out = rebase.write_topic_post_fixture(fixture, created_at)

    assert out.name == "mega_jetstream_20260722_100001.db.zip"
    with zipfile.ZipFile(out) as zf:
        (member,) = [m for m in zf.namelist() if m.endswith(".db")]
        extracted = tmp_path / "topic-check.db"
        extracted.write_bytes(zf.read(member))

    conn = sqlite3.connect(extracted)
    try:
        row = conn.execute(
            "SELECT at_uri, did, time_us, raw_post, inferences FROM enriched_posts"
        ).fetchone()
    finally:
        conn.close()

    assert row is not None
    at_uri, did, time_us, raw_post, inferences = row
    assert at_uri == fixture["at_uri"]
    assert did == fixture["did"]
    assert time_us == int(created_at.timestamp() * 1_000_000)
    message = json.loads(raw_post)["message"]
    assert message["commit"]["record"]["createdAt"] == "2026-07-22T10:00:01Z"
    assert "reply" not in message["commit"]["record"]
    assert json.loads(inferences) == fixture["inferences"]


def test_main_appends_the_topic_post_and_records_it_in_the_manifest(tmp_path, monkeypatch):
    fixtures_dir = tmp_path / "fixtures"
    fixtures_dir.mkdir()
    source_time = dt.datetime.now(dt.UTC).replace(microsecond=0) - 2 * HOUR
    source = fixtures_dir / f"mega_jetstream_{source_time:%Y%m%d_%H%M%S}.db.zip"
    _make_db_zip(source, source_time.isoformat(), int(source_time.timestamp() * 1_000_000))
    (fixtures_dir / "manifest.json").write_text(
        json.dumps({"window_end": source_time.isoformat(), "personas": []})
    )
    ...
    fixture = rebase.load_topic_post_fixture()
    assert manifest["topic_fixture"]["at_uri"] == fixture["at_uri"]
    assert manifest["topic_fixture"]["scores"]["News & Social Concern"] == 0.73779296875

    archives = sorted(out_dir.glob("*.db.zip"))
    assert len(archives) == 2
    with zipfile.ZipFile(archives[-1]) as zf:
        (member,) = [m for m in zf.namelist() if m.endswith(".db")]
        topic_db = tmp_path / "main-topic-check.db"
        topic_db.write_bytes(zf.read(member))
    conn = sqlite3.connect(topic_db)
    try:
        (at_uri, inferences) = conn.execute(
            "SELECT at_uri, inferences FROM enriched_posts"
        ).fetchone()
    finally:
        conn.close()
    assert at_uri == fixture["at_uri"]
